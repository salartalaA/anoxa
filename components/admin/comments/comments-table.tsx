...
import { updateCommentReportStatus } from "@/actions/admin/reports";
...
import { CommentReportReviewDialog } from "./comment-report-review-dialog";
...
export default function CommentsTable({
  ...
  const [reports, setReports] = useState(reportedComments);

  const [selectedReport, setSelectedReport] = useState<ReportedComment | null>(
    null
  );

  const [reviewOpen, setReviewOpen] = useState(false);

  const handleReviewReport = async (report: ReportedComment) => {
    let updatedReport = report;

    if (report.reportStatus === "PENDING") {
      await updateCommentReportStatus(report.id, "UNDER_REVIEW");
  ...
  const handleRejectReport = async (report: ReportedComment) => {
  ...
  const handleResolveReport = async (report: ReportedComment) => {
